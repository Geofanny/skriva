  ...
  <body class="min-h-screen bg-gradient-to-br from-teal-700 to-cyan-900 flex items-center justify-center relative overflow-hidden">
    ...
    <div class="bg-white/10 backdrop-blur-md p-10 rounded-xl shadow-lg w-full max-w-md relative z-10">
      <div class="flex flex-col items-center">
        ...
        <p class="text-gray-300 mb-6 text-center text-lg italic">Selamat datang kembali</p>

        @if(session('error'))
          <div class="bg-red-500 text-white px-4 py-2 rounded w-full text-center mb-4">
            {{ session('error') }}
          </div>
        @endif
        ...
        <form class="w-full" method="POST" action="/login">
          ...
          <div class="flex items-center justify-between mb-4">
            ...
            <a href="#" class="text-green-200 text-sm hover:underline">Lupa sandi?</a>
          </div>

          <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white font-semibold py-3 rounded transition duration-300">Masuk</button>
        </form>
      </div>
    </div>
